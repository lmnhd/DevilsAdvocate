        ...
        try {
          ...
          for await (const event of orchestrator.orchestrateStream({ claim, maxTokens })) {
            ...
            if (event.type === 'believer_complete') {
              ...
              console.log(`[STREAM] 🔍 Pattern 1 (VERDICT section) match: "${verdictMatch?.[1] || 'NO MATCH'}"`);
              
              if (!verdictMatch) {
                verdictMatch = verdictText.match(/^(.+?)(?:\n|\*\*CONFIDENCE)/im);
                console.log(`[STREAM] 🔍 Pattern 2 (first line) match: "${verdictMatch?.[1] || 'NO MATCH'}"`);
              }
              
              const confidenceMatch = verdictText.match(/\*\*CONFIDENCE SCORE\*\*:?\s*(\d+)/i);
              console.log(`[STREAM] 🔍 Confidence pattern match: "${confidenceMatch?.[1] || 'NO MATCH'}"`);
              
              const believerMatch = verdictText.match(/\*\*STRENGTH OF BELIEVER CASE\*\*:?\s*(\w+(?:\s+\w+)?)/i);
              console.log(`[STREAM] 🔍 Believer strength pattern match: "${believerMatch?.[1] || 'NO MATCH'}"`);
              
              const skepticMatch = verdictText.match(/\*\*STRENGTH OF SKEPTIC CASE\*\*:?\s*(\w+(?:\s+\w+)?)/i);
              console.log(`[STREAM] 🔍 Skeptic strength pattern match: "${skepticMatch?.[1] || 'NO MATCH'}"`);
              
              const harmMatch = verdictText.match(/If we acted on the Believer's position and they're wrong, what harm could result\?[^\n]*?([^\n]+?)(?=If we rejected|\*\*|$)/is);
              console.log(`[STREAM] 🔍 Harm pattern match: "${harmMatch?.[1]?.trim() || 'NO MATCH'}"`);
              ...
              console.log(`[STREAM] 🔍 Opportunity pattern match: "${opportunityMatch?.[1]?.trim() || 'NO MATCH'}"`);
